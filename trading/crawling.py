...
def crawle_earning_data() :
    specific_time.clear()
    earning_company_name.clear()

    elements = driver.find_elements(By.CLASS_NAME, "accept-policy")

    if elements:
        driver.find_element(By.CLASS_NAME, "accept-policy").click()
    ...
    return specific_time, earning_company_name

crawle_earning_data()
before_total_crawle = {key: value for key, value in zip(earning_company_name,specific_time)}

move_after_page = driver.find_element(By.XPATH, "/html/body/div[1]/main/div[5]/div/div/div/div/div[2]/a").click()

crawle_earning_data()
after_total_crawle = {key: value for key, value in zip(earning_company_name,specific_time)}

print("장 전 발표:", before_total_crawle)
print("장 중 발표:", after_total_crawle)
